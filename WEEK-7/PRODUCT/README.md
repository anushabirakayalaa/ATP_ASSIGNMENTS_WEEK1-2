...
-----------------
            ...
            -> Vite + React project setup with Tailwind v4 
            -> JSX — it's just JavaScript, not HTML
            -> Components — everything is a function that returns UI
            -> Props — just function arguments
            -> Conditional rendering and lists with .map()

Hands-on:
---------
        🛍️ Project — Products Page
        What You're Building : A clean Products Listing Page that displays 10 tech products as cards — just like you see on any real e-commerce site.
...
------------------------
      ...
      Product (Child)
            * Receives one product as a prop
            * Displays image, name, brand, description and price as a card
...
Ground Rules
-----------
        Products array must live in the parent only — not in the child
        Child must receive product data only through props
        One Product component must work for all 10 products — no copy pasting
